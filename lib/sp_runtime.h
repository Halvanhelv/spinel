...
static inline int sp_utf8_decode(const char*p,uint32_t*out){unsigned char c=(unsigned char)p[0];if(c<0x80){*out=c;return 1;}if(c<0xC0){*out=c;return 1;}if(c<0xE0){*out=((uint32_t)(c&0x1F)<<6)|((unsigned char)p[1]&0x3F);return 2;}if(c<0xF0){*out=((uint32_t)(c&0x0F)<<12)|(((uint32_t)(unsigned char)p[1]&0x3F)<<6)|((unsigned char)p[2]&0x3F);return 3;}*out=((uint32_t)(c&0x07)<<18)|(((uint32_t)(unsigned char)p[1]&0x3F)<<12)|(((uint32_t)(unsigned char)p[2]&0x3F)<<6)|((unsigned char)p[3]&0x3F);return 4;}
...
static mrb_int sp_str_length(const char*s){mrb_int n=0;while(*s){s+=sp_utf8_char_len((unsigned char)*s);n++;}return n;}
static mrb_int sp_str_ord(const char*s){if(!*s)return 0;uint32_t cp;sp_utf8_decode(s,&cp);return(mrb_int)cp;}
static size_t sp_utf8_byte_offset(const char*s,mrb_int char_idx){const char*p=s;while(char_idx>0&&*p){p+=sp_utf8_char_len((unsigned char)*p);char_idx--;}return(size_t)(p-s);}
...
static void sp_str_split_into(sp_StrArray*a,const char*s,const char*sep){
  ...
  if(sl==0){const char*p=s;while(*p){int cn=sp_utf8_char_len((unsigned char)*p);char*c=sp_str_alloc_raw(cn+1);memcpy(c,p,cn);c[cn]=0;sp_StrArray_push(a,c);p+=cn;}return;}
...
static sp_StrArray*sp_str_split(const char*s,const char*sep){sp_StrArray*a=sp_StrArray_new();if(*s==0)return a;size_t sl=strlen(sep);if(sl==0){const char*p=s;while(*p){int cn=sp_utf8_char_len((unsigned char)*p);char*c=sp_str_alloc_raw(cn+1);memcpy(c,p,cn);c[cn]=0;sp_StrArray_push(a,c);p+=cn;}return a;}const char*p=s;while(1){const char*f=strstr(p,sep);if(!f){char*r=sp_str_alloc_raw(strlen(p)+1);strcpy(r,p);sp_StrArray_push(a,r);break;}size_t n=f-p;char*r=sp_str_alloc_raw(n+1);memcpy(r,p,n);r[n]=0;sp_StrArray_push(a,r);p=f+sl;}return a;}
...
static mrb_int sp_str_index(const char*s,const char*sub){const char*f=strstr(s,sub);if(!f)return -1;mrb_int n=0;const char*p=s;while(p<f){p+=sp_utf8_char_len((unsigned char)*p);n++;}return n;}
static mrb_int sp_str_rindex(const char*s,const char*sub){size_t sl=strlen(sub);if(sl==0)return sp_str_length(s);const char*last=NULL;const char*p=s;while((p=strstr(p,sub))){last=p;p++;}if(!last)return -1;mrb_int n=0;const char*q=s;while(q<last){q+=sp_utf8_char_len((unsigned char)*q);n++;}return n;}
...
static const char*sp_str_reverse(const char*s){size_t bl=strlen(s);char*r=sp_str_alloc_raw(bl+1);size_t end=bl;const char*p=s;while(*p){int cn=sp_utf8_char_len((unsigned char)*p);end-=cn;memcpy(r+end,p,cn);p+=cn;}r[bl]=0;return r;}
