...
/* Check if a built-in type has a known method (for respond_to? resolution) */
bool builtin_has_method(spinel_type_t kind, const char *name) {
  ...
  static const char *universal[] = {
    "nil?", "is_a?", "respond_to?", "to_s", "class",
    "freeze", "frozen?", "dup", "==", "!=", "equal?", NULL
  };
  for (const char **p = universal; *p; p++)
    if (strcmp(name, *p) == 0) return true;

  switch (kind) {
  case SPINEL_TYPE_INTEGER: {
    static const char *m[] = {
      "+", "-", "*", "/", "%", "**", "<", ">", "<=", ">=",
      "&", "|", "^", "<<", ">>", "~", "-@",
      "abs", "even?", "odd?", "zero?", "positive?", "negative?",
      "to_i", "to_f", "chr", "succ", "itself", "clamp",
      "times", "upto", "downto", "divmod", NULL
    };
    for (const char **p = m; *p; p++) if (strcmp(name, *p) == 0) return true;
    break;
  }
  case SPINEL_TYPE_FLOAT: {
    static const char *m[] = {
      "+", "-", "*", "/", "%", "**", "<", ">", "<=", ">=", "-@",
      "abs", "ceil", "floor", "round", "to_i", "to_f",
      "zero?", "positive?", "negative?", "infinite?", "nan?", NULL
    };
    for (const char **p = m; *p; p++) if (strcmp(name, *p) == 0) return true;
    break;
  }
  case SPINEL_TYPE_STRING: {
    static const char *m[] = {
      ...
      "start_with?", "end_with?", "match?", "count", "index",
      "upcase", "downcase", "capitalize", "reverse",
      "strip", "lstrip", "rstrip", "chomp",
      "gsub", "sub", "tr", "delete", "squeeze",
      "split", "chars", "bytes", "each_line", "each_char",
      "ljust", "rjust", "center", "slice",
      "to_i", "to_f", "to_sym", "ord", "hex", "oct",
      "getbyte", "setbyte", "bytesize", "encode",
      "replace", "<<", "clear", NULL
    };
    for (const char **p = m; *p; p++) if (strcmp(name, *p) == 0) return true;
    break;
  }
  case SPINEL_TYPE_ARRAY:
  case SPINEL_TYPE_FLOAT_ARRAY: {
    static const char *m[] = {
      "[]", "[]=", "push", "<<", "pop", "shift", "unshift",
      "length", "size", "empty?", "first", "last", "include?",
      "each", "each_with_index", "map", "select", "reject",
      "find", "any?", "all?", "none?", "count", "sum",
      "min", "max", "sort", "sort!", "reverse", "reverse!",
      "uniq", "uniq!", "flatten", "compact", "join", "dup",
      "insert", "reduce", "inject", "sort_by", "zip", NULL
    };
    for (const char **p = m; *p; p++) if (strcmp(name, *p) == 0) return true;
    break;
  }
  case SPINEL_TYPE_HASH:
  case SPINEL_TYPE_RB_HASH: {
    static const char *m[] = {
      "[]", "[]=", "has_key?", "key?", "delete",
      "keys", "values", "each", "length", "size", "empty?",
      "merge", "transform_values", NULL
    };
    for (const char **p = m; *p; p++) if (strcmp(name, *p) == 0) return true;
    break;
  }
